...
int refactorEmbargo(int card, int choice1, struct gameState *state, int handPos) {

  	int currentPlayer = whoseTurn(state);

      //+2 Coins regular, add 20 for refactor
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
		...
		refactorAdventurer(card, state, handPos);
	...
	refactorSmithy(card, state, handPos);
		
    case village:
    
		refactorVillage(card, state, handPos);
    ...
    case great_hall:
		refactorGreatHall(card, state, handPos);
    ...
    case embargo: 
		refactorEmbargo(card, choice1, state, handPos);
